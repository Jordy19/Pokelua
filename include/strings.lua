translate.text = {
    en = {
        forum_thread = "<font color='#2E72CB'>http://atelier801.com/topic?f=6&t=838708</font>",
        infobar = "<p align='center'><j>PokeLua <bl>[<v>EN<bl>] • <font size='9'><bl>(<n>Version: <v>&version&<bl>)</font><font size='11'> <j>• <vp>!pikachu</font> <j>• <a href='event:explore.random'><font size='9'><bl>[<v>Random Pokémon<bl>]</font></a></p>",
        debug_room = "<font color='#cb546b'>• You entered a #pokelua <b>Debug Room</b>.</font>",
        test_room = "<font color='#f89406'>• You entered a #pokelua <b>Test Room</b>.</font>",
        test_room_greet = "<font color='#f89406'>• Please note that features in this room can break at any time.</font>",
        player_introduction = "<j>Welcome to public test version of <b>PokeLua</b>.",
        no_mega_capability = "<VP>You need a mega-capable Pokémon if you want to use mega-evolve with !mega",
        fanart_available = "<VP>This Pokémon is also available in a fan-art variant, try the G button.",
        command_disabled = "This command has disabled for this game-mode (%s).",
        quality_control_start = "<font color='#C53DFF'>Quality control started.</font>",
        quality_control_stop = "<font color='#d87cff'>Quality control stopped.</font>",
        developers_mode_enabled = "<font color='#d87cff'>Developers Mode enabled.</font>",
        developers_mode_disabled = "<font color='#d87cff'>Developers Mode disabled.</font>",
        new_admin = "New room administrator: <b>%s</b>",
        admin_message = "<font color='#EB1D51'>~ [<b>A</b>]</font><font color='#CB2655'> %s</font>",


    },
    xx = en,
    nl = {
        infobar = "<p align='center'><j>PokeLua <bl>[<v>NL<bl>] • <font size='9'><bl>(<n>Versie: <v>&version&<bl>)</font><font size='11'> <j>• <vp>!pikachu</font> <j>• <a href='event:explore.random'><font size='9'><bl>[<v>Willekeurige Pokémon<bl>]</font></a></p>",
    }
}
-- For international, e2 community we will fall back to EN.
translate.text.xx = translate.text.en
translate.text.e2 = translate.text.en