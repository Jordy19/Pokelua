...
def moduleFile(path, fileTree, fileOrder, srcPath):
    ...
    tree = [file for file in fileOrder if file in fileTree]    for file in tree:
        filePath = "%s\%s.lua" % (srcPath, file.replace(".", "\\"))
        new_file = parseFile(filePath)
        new_file = new_file.replace('&version&', '5.0')
        f.write(new_file)
    f.write("\ninit()")
    f.close()
